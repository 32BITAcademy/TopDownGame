...
void GameManager::ReadMsgs()
{
	...
	while (!msgs.empty())
	{
		...
		if (m.type == MSG_SHOOT)
		{
			...
			{
			case OBJ_BULLET:
				AddObject(new Bullet((Unit*)m.sender, 10, DT_STANDARD, speed, 5000, m.shoot.pos));
				break;
			case OBJ_ROCKET:
				AddObject(new Rocket({100,100}, (Unit*)m.sender, 10, DT_STANDARD,speed,5000, m.shoot.pos));
				break;
			}
			continue;
		}
